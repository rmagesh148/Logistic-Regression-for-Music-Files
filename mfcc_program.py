# -*- coding: utf-8 -*-
"""
Created on Wed Apr 08 18:01:16 2015
...
final_input=np.load('F:/ceps_data.npy') ## change the file path to ceps_data.NPY file mfcc data, please read the readme file
class_matrix_final=np.load('F:/label.npy') ##class files for the classifier
training_input=[];
testing_input=[];
...
for value in range (0,len(kf)):
    ...
    weights=np.zeros((6,14))#weights initializing to zeros for 14 features
                ...
                if (values_delta[i]==2):
                    ...
                    if (values_delta[i]==3):
                        ...
                        if (values_delta[i]==4):
                            ...
                            if (values_delta[i]==5):
                                delta_matrix[5,i]=1
    for j in range(0,2000):#2000 Epochs for training the dataset
        neta=neta/(1+(j/2000)) ##calculation of neta value
        output_matrix=weights.dot(training_data_t) #Logistic Regression Method to find the P(Y|X)
    ...
    input_test_matrix=np.ones((60,14))
