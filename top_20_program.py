# -*- coding: utf-8 -*-
"""
Created on Fri Apr 10 18:41:15 2015
...
#Program for top 20 category
final_input=np.load('F:/top_20.npy')#path to the file to top_20.NPY- please read the readme file, I have already used top_20.py to extract the top 20 features and used it here
class_matrix_final=np.load('F:/class_label.npy') #change the path to the NPY file - class labels matrix
...
for value in range (0,len(kf)):
    ...
    weights=np.zeros((6,121)) #weights initializing to zeros only 120 features and bias
    shuffle_train=np.random.permutation(training_input[value])
    shuffle_test=np.random.permutation(testing_input[value])#used random permutation technique to shuffle the folded data and storing the indices of it
                ...
                if (values_delta[i]==2):
                    ...
                    if (values_delta[i]==3):
                        ...
                        if (values_delta[i]==4):
                            ...
                            if (values_delta[i]==5):
                                delta_matrix[5,i]=1
    for j in range(0,2000):#2000 Epochs used for training
    ...
    max_value_test=np.max(testing_data,axis=0)#Normalizing the test data, so finding the maximum of each rows
    testing_data=np.true_divide(testing_data,max_value_test) #Normalized test data
    input_test_matrix=np.ones((60,121))
